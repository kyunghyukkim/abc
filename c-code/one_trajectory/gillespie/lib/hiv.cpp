...
//fourier tr is not working correctly.
#include <iostream>
#include <fstream>
#include <sstream>
#include <string>
...
#include <time.h>
#include <cstring>
#include "/home/kkim/research/lib/mt19937ar-cok.cpp"
#include "/home/kkim/research/lib/corfun.cpp"
#include "/home/kkim/research/lib/average.cpp"
...
int main(void)
{
	...
	//Evolve the model up to n=NGRID_BF_STEADY
	time_evol();
	//Compute autocorrelations, crosscorrelations, probability distribution functions, flux-average-standard-deviation, concentration-average-standard-deviation, etc.
	time_ave();
	MC_results();
...
void time_evol(void)
{
	...
	FILE_TEVOL.open("tevol", ios::out);
	FILE_FLUX.open("v_t", ios::out);
...
void MC_results(void)
{
	...
	for(int i=1;i<=NUM_SPEC;i++){
		...
		for(j=0;j<=SMAX[i];j++) 
			myfile<<j<<"\t"<<(double)counterS[i][j]/counter<<endl;
		myfile.close();
	}



	myfile.open("s-ave-std", ios::out);
	for(i=1;i<=NUM_SPEC;i++)
		myfile<<i<<"\t"<<aveS[i]<<"\t"<<stdS[i]<<endl;
	myfile.close();

	myfile.open("J-ave-std", ios::out);
	for(i=1;i<=NUM_REACT;i++)
		myfile<<i<<"\t"<<aveJ[i]<<"\t"<<sqrt(covJ[i][i])<<endl;
	myfile.close();

	myfile.open("J-cov", ios::out);
	myfile<<"#row_index column_index J-cov"<<endl;
	for(i=1;i<=NUM_REACT;i++)
		for(j=1;j<=NUM_REACT;j++)
			myfile<<i<<"\t"<<j<<"\t"<<covJ[i][j]<<endl;
	myfile.close();
...
/*
	...
	for(i=1;i<=NUM_SPEC;i++){
		for(j=i;j<=NUM_SPEC;j++){
			...
			myfile<<"#frequency"<<"\t"<<"psd"<<endl;
			for(k=0;k<=n_point_fourier/2;k++)
				myfile<<1./n_point_fourier/GRID_CONST*k<<"\t"<<onepsd[i][j][k]<<endl;
			myfile.close();
		}
...
unsigned long  TCORGRID_mult_of_2(void)
{
	...
	if(tcorgrid<2) tcorgrid=0;
		...
		cout<< "TCORGRID is too large so increase GRID_CONT"<<endl;
		exit(-1);	
	}
	return tcorgrid;
}
...
bool  MC_not_finished(int time_evolution)
{
	...
	if(time_evolution==1){
		...
		while(TIME>INDEX_GRID*GRID_CONST) {
			...
			Qhead++;
//			cout<<INDEX_GRID<<"\t"<<counter<<"\t"<<S[1]<<"\t"<<S[2]<<endl;
